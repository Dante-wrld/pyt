...
ROLE_BOUNDARIES: dict[AgentRole, str] = {
    AgentRole.OPPORTUNITY_HUNTER: (
        ...
        "deterministic bar for consecutive_confirmations consecutive "
        "cycles (see closed_position for its exit price, growth since "
        "exit, and confirmations_required) - it has no recovery_review or "
        "entry_confirmation fields, since that comes from a different, "
        "simpler check based on a single price quote rather than the "
        "recommendation engine's own richer tracking, so treat it as "
        "weaker evidence than a fresh candidate showing the same "
        "confirmation count; weigh closed_position and mint_trade_history "
